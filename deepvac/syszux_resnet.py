...
from .syszux_deepvac import Deepvac, DeepvacTrain
from .syszux_loader import OsWalkDataset, FileLineDataset
...
class ResNet50Test(Deepvac):
    ...
    def validate(self, t, img_path):
        ...
        if img.shape is None:
            print('illegal image detected in validate!')
        ...
        print('Overall process time in validate: {} | Index: {} | Class: {} | Probability: {}'.format(end-start, max_index, max_index, max_probability))
        print('---------------VALIDATE END---------------\n')
    ...
    def warmUp(self, t):
        ...
        for i in range(10):
            ...
            print('Overall process time in warmup: {}'.format(end-start))
    ...
    def benchmark(self, t, img_path):
        ...
        item_num = t.size(0)
        print('---------------BENCHMARK BEGIN--------------- {}'.format(item_num))
        ...
        for i in range(item_num):
            ...
            torch.cuda.synchronize()
            tock = time.time()
            print('forward:model forward time: {}'.format(tock-tick))
    ...
    def process(self):
        for input_tensor, path in self.test_loader:
            ...
            max_res = torch.max(softmaxs, dim=1)
            max_probability, max_index = max_res
            print(path[0], max_probability.item(), max_index.item())
